...
public class AllTypeVisitorModificationPoint extends ASTVisitor {
    ...
    public boolean visit(ForStatement node) {
        ...
        if ((node.initializers().size()>0)&&(node.initializers().get(0) instanceof VariableDeclarationExpression)) {
            VariableDeclarationExpression expression1 = (VariableDeclarationExpression) node.initializers().get(0);
            for (Object obj : expression1.fragments()) {
                VariableDeclarationFragment v = (VariableDeclarationFragment) obj;
                Type varType = expression1.getType();
                String varName = v.getName().toString();

                ExpressionInfo expressionInfo = new ExpressionInfo(v.getName(),methClaPacOfExpName,lineNode,varType,varName);
                list.add(expressionInfo);
            }
        }
